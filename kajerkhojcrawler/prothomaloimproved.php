...
while($categoryID<21)
	...
	while($pageID<21)
		...
		$html = file_get_html('http://www.prothom-alojobs.com/index.php?Theme=home_new_ad&Script=job_category_list&JobCategoryID='.$categoryID.'&page='.$pageID);

	
		$cells = $html->find('td[class=company_name]');
		$p=0;
		$linkcount=0;
		$resultcount=0;
		...
		if($short!="")
			{
			$short='http://www.prothom-alojobs.com'.$short;
	...
	else if($categoryID==2){$cid=2;}
	else if($categoryID==3){$cid=12;}
	else if($categoryID==4){$cid=12;}
	else if($categoryID==5){$cid=12;}
	else if($categoryID==6){$cid=12;}
	else if($categoryID==7){$cid=9;}
	else if($categoryID==8){$cid=4;}
	else if($categoryID==9){$cid=11;}
	else if($categoryID==10){$cid=7;}
	else if($categoryID==11){$cid=12;}
	else if($categoryID==12){$cid=8;}
	else if($categoryID==13){$cid=5;}
	else if($categoryID==14){$cid=12;}
	else if($categoryID==15){$cid=3;}
	else if($categoryID==16){$cid=10;}
	else if($categoryID==17){$cid=12;}
	else if($categoryID==18){$cid=12;}
	else if($categoryID==19){$cid=12;}
	else if($categoryID==20){$cid=6;}
	else {$cid=12;}
	
	$result=mysql_query("INSERT INTO posts(postedin, cid,  comname, jobtitle, link, qualification, deadline) VALUES ('prothomalo','{$cid}','{$alldata[$p]}','{$alldata[$q]}', '{$links[$t]}', '{$alldata[$r]}' , '{$alldata[$s]}')");
